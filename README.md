# Intersepec-bo

The Readme file

Pour installer python aller sur le site télécharger la version voir fichier téléchargement

Pour connaitre la version de python
-python --verion

installation de django

python -m pip install django

Vérification de la bonne installation de django
python -m django --version

Création du projet django:

(django-admin startprojec [nom du projet en question])
ici c'est:
django-admin startproject interspecbo

création d'une application dans notre projet:

python manage.py startapp [nom_de_l'application]
ici c'est :
python manage.py startapp backoffice
